...
if __name__ == "__main__":
    ...
    # This is where it_lst is initiated I should update here it_lst such that it removes its that fail / that
    # I do not want counted
    # Also need to updatate hdf5 creation / update to ensure pervious itertaions that were counted are no
    # longer included
    it_csv = sim_dir + "iteration_check.csv"
    it_df = pd.read_csv(it_csv)
    it_msk = it_df[sim] != 0
    it_skip_arr = it_df["it_id"][it_msk].values
    it_lst = np.array([it for it in it_lst if gc_utils.iteration_name(it) not in it_skip_arr])
